...
describe("Amazon Checkout Test", function () {
  let driver;
  ...
  it("should proceed to checkout and verify total price", async function () {
    ...
    await driver.findElement(By.name("proceedToRetailCheckout")).click();

    const totalPrice = await driver
      .findElement(By.id("sc-subtotal-amount-activecart"))
      .getText();
    assert(totalPrice.includes("$"));
  });
});
